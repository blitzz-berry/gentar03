            ...
            <div class="bg-white shadow-md rounded-lg p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('kategori') border-red-500 @enderror"
                            ...
                            <option value="lainnya" {{ old('kategori', $artikel->kategori) === 'lainnya' ? 'selected' : '' }}>Lainnya</option>
                        </select>
                        @error('kategori')
                            <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                        @enderror
                ...
                <div class="mb-4">
                    ...
                    @if($artikel->thumbnail)
                        <div class="mb-2">
                            <img src="{{ asset('storage/' . $artikel->thumbnail) }}" alt="{{ $artikel->judul }}" class="h-32 w-32 object-cover rounded">
                        </div>
                    @endif
                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('thumbnail') border-red-500 @enderror"
                           ...
                           name="thumbnail"
                           accept="image/*">
                    <p class="text-gray-600 text-xs italic mt-2">Format: jpeg, png, jpg (Max: 2MB). Kosongkan jika tidak ingin mengganti thumbnail.</p>
                    @error('thumbnail')
                        <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('video_url') border-red-500 @enderror"
                           ...
                           type="url"
                           name="video_url"
                           value="{{ old('video_url', $artikel->video_url) }}"
                ...
                <div class="flex items-center mb-4">
                    <label class="flex items-center cursor-pointer">
                        ...
                        <input type="checkbox" name="aktif" value="1" class="form-checkbox" {{ old('aktif', $artikel->aktif) ? 'checked' : '' }}>
